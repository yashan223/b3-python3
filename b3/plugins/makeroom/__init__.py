...
class MakeroomPlugin(Plugin):
    ...
    def onLoadConfig(self):
        ...
        try:
            self._retain_free_duration = self.config.getint('global_settings', 'retain_free_duration')            self.info('global_settings/retain_free_duration: %s' % self._retain_free_duration)
    ...
    def load_config_automation(self):
        ...
        try:
            self._total_slots = self.config.getint('automation', 'total_slots')            self.info('automation/total_slots: %s' % self._total_slots)
        except (NoOptionError, ValueError) as err:
            ...
            if self._total_slots < 2:
                ...
                return
            try:
                self._min_free_slots = self.config.getint('automation', 'min_free_slots')                self.info('automation/min_free_slots: %s' % self._min_free_slots)
            except (NoOptionError, ValueError) as err:
