...
class JumpRun(object):
    ...
    def unlinkdemo(self):
        ...
        if self.demo is None:
            ...
            try:
                os.unlink(path)
                self.p.debug('removed jumprun demo file : %s' % path)            except os.error as e:
                # when this happen is mostly a problem related to misconfiguration
                errno, errstr = e.args if len(e.args) >= 2 else (e.args[0] if e.args else 0, str(e))
...
class JumperPlugin(b3.plugin.Plugin):
    ...
    ####################################################################################################################    def cmd_record(self, data, client, cmd=None):
        ...
        for jumprun in records:
            ...
            cmd.sayLoudOrPM(client, self.getMessage('client_record_pattern', {'way': wi, 'time': tm, 'date': dt}))    def cmd_maprecord(self, data, client, cmd=None):
        """
        ...
        for jumprun in records:
            ...
            cmd.sayLoudOrPM(client, self.getMessage('map_record_pattern', {'way': wi, 'client': jumprun.client.name,
                                                                           'id':  jumprun.client.id, 'time': tm}))    def cmd_topruns(self, data, client, cmd=None):
        """
        ...
        for jumprun in records:
            ...
            cmd.sayLoudOrPM(client, self.getMessage('map_toprun_pattern', {'way': way_id, 'place': place,
                                                                           ...
                                                                           'id': jumprun.client.id, 'time': way_time}))    def cmd_delrecord(self, data, client, cmd=None):
        """
    ...
    def cmd_setway(self, data, client, cmd=None):
        ...
        cursor.close()    def cmd_map(self, data, client, cmd=None):
        """
    ...
    def cmd_pasetnextmap(self, data, client=None, cmd=None):
        ...
        if isinstance(match, list):
            client.message('^7do you mean: %s?' % ', '.join(match[:5]))
            return        if isinstance(match, str):
