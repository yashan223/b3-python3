...
try:
    ...
    class imp:
        ...
        def find_module(name, path=None):
            if path:
                spec = importlib.util.spec_from_file_location(name, os.path.join(path[0], name + '.py'))
            else:
...
class Parser(object):
    ...
    def __init__(self, conf, options):
        ...
        log2console = self.config.has_option('devmode', 'log2console') and \
            self.config.getboolean('devmode', 'log2console')        # make sure the logfile is writable
        ...
        if path is not None:
            ...
            try:
                ...
                if os.path.isdir(plugin_path) and os.path.isfile(os.path.join(plugin_path, '__init__.py')):
                    spec = importlib.util.spec_from_file_location(name, os.path.join(plugin_path, '__init__.py'))
                    if spec is None:
                        raise ImportError(f"No module named '{name}'")
                    module = importlib.util.module_from_spec(spec)
                    spec.loader.exec_module(module)
                    ...
                    if os.path.isfile(plugin_file):
                        ...
                        raise ImportError(f"Plugin '{name}' not found at path '{path}'")
            except Exception as e:
                raise ImportError(f"Error loading plugin '{name}' from path '{path}': {e}")

        # Try built-in plugins first
        try:
            plugins_dir = os.path.join(b3.getB3Path(True), 'plugins')
            ...
            if os.path.isdir(plugin_path) and os.path.isfile(os.path.join(plugin_path, '__init__.py')):
                ...
                # Try as .py file 
                plugin_file = os.path.join(plugins_dir, name + '.py')
                if os.path.isfile(plugin_file):
                    spec = importlib.util.spec_from_file_location(name, plugin_file)
                    if spec is None:
                        raise ImportError(f"No module named '{name}'")
                    ...
                    raise ImportError(f"Plugin '{name}' not found in built-in plugins")
            ...
            ext_plugins_dir = self.config.get_external_plugins_dir()
            plugin_path = os.path.join(ext_plugins_dir, name)
            ...
            if os.path.isdir(plugin_path) and os.path.isfile(os.path.join(plugin_path, '__init__.py')):
                ...
                # Try as .py file
                plugin_file = os.path.join(ext_plugins_dir, name + '.py')
                if os.path.isfile(plugin_file):
                    spec = importlib.util.spec_from_file_location(name, plugin_file)
                    if spec is None:
                        raise ImportError(f"No module named '{name}'")
                    ...
                    raise ImportError(f"Plugin '{name}' not found in external plugins directory")
